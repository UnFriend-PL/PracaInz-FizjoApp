...
const MedicalSurvey = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const emptyTextAreas = questions
      ...
      .filter(
        (q) =>
          !formData[`question-${q.followUp.id}`] ||
          formData[`question-${q.followUp.id}`].trim() === ""
      )
      .map((q) => q.followUp.id);
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        {questions.map((question) => (
          <div key={question.id} className={styles.questionContainer}>
            ...
            {formData[`question-${question.id}`] === "yes" &&
              question.followUp && (
                <div
                ...
                >
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    ref={(el) =>
                      (textAreasRef.current[question.followUp.id] = el)
                    }
                  />
                  {textAreaErrors[question.followUp.id] && (
                  ...
                  <div className={styles.charCount}>
                    {charCounts[`question-${question.followUp.id}`] || 0} /{" "}
                    {MAX_TEXT_LENGTH} znaków
                  </div>
                </div>
              )}
          </div>
          ...
          <div className={styles.agreementItem}>
            <label className={styles.checkboxLabel}>
              ...
              <span className={styles.checkboxText}>
                ...
                przeze mnie odpowiedzi są zgodne ze stanem faktycznym. Pomimo
                występowania wymienionych przeze mnie w pkt. 1 chorób oraz/lub
                potwierdzam chęć skorzystania z usługi masażu na własną
                odpowiedzialność.
              </span>
            </label>
          </div>
          ...
          <div className={styles.agreementItem}>
            <label className={styles.checkboxLabel}>
              ...
              <span className={styles.checkboxText}>
                ...
                usługi masażu oferowanego przez [nazwa firmy], a także z
                przeciwwskazaniami oraz wpływem masażu na organizm człowieka.
              </span>
            </label>
          </div>
          <div className={styles.agreementItem}>
            <label className={styles.checkboxLabel}>
              ...
              <span className={styles.checkboxText}>
                ...
                mojego stanu zdrowia, która nastąpi w trakcie udzielania
                zabiegów.
              </span>
            </label>
          </div>
